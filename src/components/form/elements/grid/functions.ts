...
}) { /* TODO: We can probably create an "edit" mode we're just disable these so the user isn't thrown out of the input when trying to type (if we are not in edit mode we overwrite existing data when typing maybe? see google docs...) */
  if (e.key === 'ArrowDown') {
    e.preventDefault()
