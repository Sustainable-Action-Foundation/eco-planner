...
export default function goalsToTree(goals: ((Goal & { dataSeries: DataSeries | null } & { roadmap: { id: string, metaRoadmap: { name: string, id: string } } }) | null)[], t: TFunction) {
  ...
  for (const goal of sortedGoals) {
    ...
    current[`${goal.name || goal.indicatorParameter.split('\\').slice(-1)} (\u200c${goal.dataSeries?.unit === null ? t("common:tsx.unitless") : goal.dataSeries?.unit || t("common:tsx.unit_missing")})`] = goal;
  }
