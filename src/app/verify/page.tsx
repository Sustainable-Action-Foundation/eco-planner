  ...
  return (
    <main>
      <div className="margin-block-300 padding-inline-100 padding-bottom-100 container-text margin-inline-auto purewhite smooth" style={{border: '1px solid var(--gray)'}}>
        <h1 className="padding-bottom-100" style={{borderBottom: '1px solid var(--gray)'}}>{dict.verifyEmail}</h1>
        <p>{dict.emailVerification}</p>
        <h2 className="margin-top-200 margin-bottom-50" style={{fontSize: '1.25rem'}}>{dict.noEmailReceived}</h2>
        <p className="margin-top-0">{dict.retryInfo}</p>
        <form onSubmit={handleSubmit} className="flex gap-50 flex-wrap-wrap align-items-center">
          ...
          <button type="submit" className="font-weight-500" style={{fontSize: '1rem', minHeight: 'calc(24px + 1rem)'}}>{dict.sendNewEmail}</button>
        </form>
      </div>
    </main>
  )
