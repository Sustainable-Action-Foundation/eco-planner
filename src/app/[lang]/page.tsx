...
import { getDictionary, type dictionaries } from "./dictionaries";

export default async function Page({ searchParams, params }: { searchParams: { [key: string]: string | string[] | undefined }; params: { lang: string } }) {
  const [session, metaRoadmaps, dict] = await Promise.all([getSession(cookies()), getMetaRoadmaps(), getDictionary(params.lang as keyof typeof dictionaries)]);
  ...
  metaRoadmaps.forEach((metaRoadmap) => {
    if (metaRoadmap.roadmapVersions.length) {
      // ? Is this the line that gets the latest version of the roadmap?
  ...
  if (searchFilter) {
    roadmaps = roadmaps.filter((roadmap) => {
      if (
        Object.values(roadmap).some((value) => {
        ...
        Object.values(roadmap.metaRoadmap).some((value) => {
          if (typeof value === "string") {
            return value.toLowerCase().includes(searchFilter.toLowerCase());
          } else {
            return false;
          ...
          <div className="flex gap-100 flex-wrap-wrap align-items-flex-end justify-content-space-between padding-100 width-100">
            <div>
              ...
              <p className="margin-0">
                Photo by{" "}
                <a className="color-purewhite" href="https://unsplash.com/@markusspiske?utm_content=creditCopyText&utm_medium=referral&utm_source=unsplash" target="_blank">
                  Markus Spiske
                </a>{" "}
                on{" "}
                <a
                  className="color-purewhite"
                  href="https://unsplash.com/photos/white-and-blue-solar-panels-pwFr_1SUXRo?utm_content=creditCopyText&utm_medium=referral&utm_source=unsplash"
                  target="_blank">
                  Unsplash
                </a>
              </p>
            </div>
            {
              ...
              session.user && (
                <>
                  <a href="/metaRoadmap/create" className="button purewhite round block">
                    Skapa ny färdplansserie
                  </a>
