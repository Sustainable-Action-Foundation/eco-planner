import dataSeriesPrep from "@/app/[lang]/api/goal/dataSeriesPrep";
...
export async function POST(request: NextRequest) {
    const [session, effect] = await Promise.all([getSession(cookies()), request.json() as Promise<JSONValue>]);
    ...
    if (!session.user?.id) {
        return Response.json({ message: "Unauthorized" }, { status: 401, headers: { Location: "/login" } });
    }
    ...
    if (dataSeries == null) {
        return Response.json({ message: "Bad data series" }, { status: 400 });
    }
    ...
    try {
        ...
        return Response.json({ message: "Effect created", actionId: newEffect.actionId, goalId: newEffect.goalId }, { status: 201 });
    } catch (error) {
        // Unique constraint error
        if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
            return Response.json({ message: "Effect already exists, try edit page if you want to change values" }, { status: 409 });
        }
        console.log(error);
...
export async function PUT(request: NextRequest) {
    ...
    function isEffect(effect: JSONValue): effect is EffectInput & { timestamp: number } {
        return (
            ...
            (effect.dataSeries === undefined || (effect.dataSeries instanceof Array && effect.dataSeries.every((value) => typeof value === "string"))) &&
            // impactType may be included, and should in that case be one of the values in ActionImpactType
            (effect.impactType === undefined || Object.values(ActionImpactType).includes(effect.impactType as ActionImpactType)) &&
        ...
        if (error instanceof Error) {
            switch (error.message) {
                ...
                case ClientError.AccessDenied:
                    return Response.json({ message: ClientError.AccessDenied }, { status: 403 });
                case ClientError.StaleData:
                    return Response.json({ message: ClientError.StaleData }, { status: 409 });
                default:
                    return Response.json({ message: "Unknown error" }, { status: 500 });
            }
        } else {
    ...
    try {
        ...
        revalidateTag("goal");
        // Return success
        return Response.json({ message: "Effect updated", actionId: updatedEffect.actionId, goalId: updatedEffect.goalId }, { status: 200 });
    } catch (error) {
        console.log(error);
        return Response.json({ message: "Internal server error" }, { status: 500 });
    }
...
export async function DELETE(request: NextRequest) {
    ...
    try {
        ...
        revalidateTag("goal");
        // Return success
        return Response.json({ message: "Effect deleted", actionId: deletedEffect.actionId, goalId: deletedEffect.goalId }, { status: 200 });
    } catch (error) {
        console.log(error);
        return Response.json({ message: "Internal server error" }, { status: 500 });
    }
