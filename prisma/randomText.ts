...
export class RandomTextSE {
  ...
  static sentence(wordCount: number): string {
    ...
    if (items.length === 0 && wordCount > 1) {
      console.info("No items found for word count", wordCount, "falling back to word count", wordCount - 1);
      return this.sentence(wordCount - 1)
    }
    else if (items.length === 0) {
      console.warn("No items found for word count", wordCount, "returning random word");
      return this.words(1);
    }
  ...
  static words(wordCount: number = 1): string {
    ...
    if (items.length === 0 && wordCount > 1) {
      ...
      console.warn("No items found for word count", wordCount, `returning ${this.fallbackString} in place of words`);
